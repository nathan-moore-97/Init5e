...
function handleCreateCharacterRequest(req) {
    ...
    var notes = exports.mySqlRealEscapeString(req.notes) != undefined ? exports.mySqlRealEscapeString(req.notes) : 'NULL';
    ...
    if (player != undefined &&  name != undefined && race != undefined && !isNaN(ac) && !isNaN(pp) && !isNaN(dex)) {
        return {ins: `insert into lu_character (player_name, char_name, class, race, level, ac, passive_perception, speed, spell_save, notes, dex_mod, pc, init_adv)
                    values('${player}', '${name}', '${_class}', '${race}',  ${level}, ${ac} , ${pp}, ${speed}, ${sdc}, '${notes}', ${dex}, ${pc}, ${hasAdv});`}
    }
...
function handleCreateEncounterRequest(req) {
    ...
    var notes = exports.mySqlRealEscapeString(req.notes);
...
function handleDeleteCharacterRequest(req) {
    return {err: `DELETE Character not implemented yet`}
}

function handleDeleteEncounterRequest(req) {
    return {err: `DELETE Encounter not implemented yet`}
}
...
function handleGetCharacterRequest(req) {
    ...
    if (whichId != undefined) {
        return {ins: `select * from lu_character where id = ${whichId}`}
    }
    // Was not looking for id, get by name
    var whichName = exports.mySqlRealEscapeString(req.name);
    if(whichName != undefined) {
        return {ins: `select * from lu_character where char_name = '${whichName}'`}
    }
...
function handleGetEncounterRequest(req) {
    ...
    var whichId = exports.mySqlRealEscapeString(req.id);
    if(whichId != undefined) {
        return {ins: `select * from lu_encounter where id = ${whichId}`}
    }
...
function handleGetFightRequest(req) {
    var whichId = exports.mySqlRealEscapeString(req.id);
    if(whichId != undefined) {
        return {ins: `select player_name, char_name, character_id, class, combatant_id, dex_mod, race, ac, speed, spell_save, passive_perception, init_adv, level, pc, notes
                from lu_encounter 
                join lu_fight 
